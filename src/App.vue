  ...
  <div class="app">
    <div id="tooling">
      ...
      <form>
        ...
        <br />
        <button type="submit" @click.prevent="drawShape(model)">
          Create Shape
        </button>
...
export default {
  ...
  mounted() {
    ...
    this.svgRenderer = document
      .getElementById('svg-renderer')
      .querySelector('svg');
    ...
    drawShape(model, isFromStore) {
      ...
      if (shapeObj) {
        ...
        if (
          ...
          createdShape.setAttribute(
            'style',
            `fill:${model.color};fill-rule:nonzero`
          );
        }
        createdShape.setAttribute('class', 'rubberBand');
        this.svgRenderer.appendChild(createdShape);
